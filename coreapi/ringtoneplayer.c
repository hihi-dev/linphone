...
LinphoneStatus linphone_ringtoneplayer_start(MSFactory *factory, LinphoneRingtonePlayer* rp, MSSndCard* card, int out_device_id, int in_device_id, const char* ringtone, int loop_pause_ms) {
	return linphone_ringtoneplayer_start_with_cb(factory, rp, card, out_device_id, in_device_id, ringtone, loop_pause_ms, NULL, NULL);
}
...
LinphoneStatus linphone_ringtoneplayer_start_with_cb(MSFactory *factory, LinphoneRingtonePlayer* rp, MSSndCard* card, int out_device_id, int in_device_id, const char* ringtone, int loop_pause_ms, LinphoneRingtonePlayerFunc end_of_ringtone, void * user_data) {
	if (linphone_ringtoneplayer_is_started(rp)) {
		ms_message("the local ringtone is already started");
		return 2;
	}
	if (card!=NULL && ringtone){
		...
		rp->ringstream=ring_start_with_cb(factory, ringtone,loop_pause_ms,card,out_device_id,in_device_id,notify_end_of_ringtone,rp);
