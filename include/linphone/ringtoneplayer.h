...
LINPHONE_PUBLIC LinphoneStatus linphone_ringtoneplayer_start(MSFactory *factory, LinphoneRingtonePlayer* rp, MSSndCard* card, int out_device_id, int in_device_id, const char* ringtone, int loop_pause_ms);
/**
 * Start a ringtone player
 * @param factory A MSFactory object
 * @param rp #LinphoneRingtonePlayer object
 * @param card unused argument
 * @param out_device_id the audio output device
 * @param in_device_id the audio input device (for early input stream - see HS-96)
 ...
 */
LINPHONE_PUBLIC LinphoneStatus linphone_ringtoneplayer_start_with_cb(MSFactory *factory, LinphoneRingtonePlayer* rp, MSSndCard* card, int out_device_id, int in_device_id,
